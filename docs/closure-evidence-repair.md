...
- **B. Vitest unit test** — the `*.test.ts` file under `src/workflow/**` exercising the typed `Result<T, E>` return shape at the workflow-module boundary per `AGENTS.md` line 54 (or, for tickets whose deliverable is a non-workflow deep module — e.g. Calendar Connection encryption at `tests/calendar-token-encryption.test.ts`, Admin shell migration at `tests/admin-shell-repository-migration.test.ts`, AppClock migration at `tests/appclock-boundary-migration.test.ts`, localTime contract at `src/time/local-time.test.ts`, Search assembler at `src/search/search-snapshot-assembler.test.ts` — the equivalent boundary test under `src/**` or `tests/**`).
...
- **D. Visual capture (committed)**: `tests/e2e-browser/screenshots/admin/users/` (per-state PNGs captured by the T16 capture run). The Admin Topics and Admin Status capture sets are pending T17 (#303) and T18 (#304); the row records the T17 / T18 open-blocker state and the visual-capture evidence will land when those tickets close.
- **E. Visual capture (workflow artifact)**: `workflow_dispatch` `visual-regression.yml` lane per AGENTS.md line 65.
