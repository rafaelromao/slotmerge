...
- **A.1 Playwright failure-path spec**: same file; the `needs_reconnect` Calendar Connection outcome and the reconnect flow. The action-required-email wiring boundary lives in `tests/calendar-action-required-email-wiring.test.ts` (already cited in the B-cell) rather than in this Playwright spec.
...
- **B. Vitest unit test**: `tests/retired-routes.test.ts`.
...
- **Existing `## Closure Evidence` record**: closed on 2026-07-28T04:23:19Z; the existing comment on this issue (issue comment [5099920821](https://github.com/rafaelromao/slotmerge/issues/16#issuecomment-5099920821)) is the T25-batch closure-evidence record and remains the primary record.
...
- **G. Corrective comment + Closure PR**: primary `## Closure Evidence` record at [issue #16 comment 5099920821](https://github.com/rafaelromao/slotmerge/issues/16#issuecomment-5099920821); audit-repair strengthening comment posted at [issue #16 comment 5111930110](https://github.com/rafaelromao/slotmerge/issues/16#issuecomment-5111930110) (with framing clarification at [issue #16 comment 5112191123](https://github.com/rafaelromao/slotmerge/issues/16#issuecomment-5112191123)); **no single closing PR owns this issue** (the sub-PRD closes via the T25 batch).

### C3. [#17](https://github.com/rafaelromao/slotmerge/issues/17) Sub-PRD: Calendar Connections

- **Existing `## Closure Evidence` record**: closed on 2026-07-28T04:23:21Z; the existing comment on this issue (issue comment [5099921055](https://github.com/rafaelromao/slotmerge/issues/17#issuecomment-5099921055)) is the T25-batch closure-evidence record and remains the primary record.
...
- **G. Corrective comment + Closure PR**: primary `## Closure Evidence` record at [issue #17 comment 5099921055](https://github.com/rafaelromao/slotmerge/issues/17#issuecomment-5099921055); audit-repair strengthening comment posted at [issue #17 comment 5111930172](https://github.com/rafaelromao/slotmerge/issues/17#issuecomment-5111930172) (with framing clarification at [issue #17 comment 5112191242](https://github.com/rafaelromao/slotmerge/issues/17#issuecomment-5112191242)); **no single closing PR owns this issue** (the sub-PRD closes via the T25 batch).

### C4. [#18](https://github.com/rafaelromao/slotmerge/issues/18) Sub-PRD: Admin & Notifications

- **Existing `## Closure Evidence` record**: open at the audit window (the prior `## Closure Evidence` comment on this issue (issue comment [5099921404](https://github.com/rafaelromao/slotmerge/issues/18#issuecomment-5099921404)) was posted while T17 / T18 / T19 were still open; T17 (#303) closed on 2026-07-28T13:57:40Z and T19 (#305) closed on 2026-07-28T15:07:07Z before the audit window, and T18 (#304) closed on 2026-07-28T03:58:11Z — i.e., all four T-tickets closed before the audit window — so the prior closure-evidence comment is materially correct at the audit window for the closing-time gate but the Admin Topics capture set, the Admin Status capture PNG set, and the Admin journey end-to-end screencast still land after the audit window and are partially on disk).
- **A. Playwright happy-path spec**: `tests/e2e-browser/journeys/admin/users.spec.ts` (T16 happy: invite / role change / suspend / reinstate); `tests/e2e-browser/journeys/admin/topics.spec.ts` (T17 happy: approve a Proposal, reject a Proposal, retire a Topic); `tests/e2e-browser/journeys/admin/status.spec.ts` (T18 happy: Email health, Calendar Connection summary, Tokens-needing-refresh table); `tests/e2e-browser/journeys/admin/end-to-end.spec.ts` (T19 happy: full Admin happy path).
- **A.1 Playwright failure-path spec**: same four files; T17 carries the typed-confirm Retire confirm step and the Admin-who-proposed-cannot-retire self-action protection failure path; T18 carries the warning-banner failure paths (`Emails failed in the past 24h` > 5%, `needs_reconnect` > 1); T19 carries the no-`Refresh now`-button assertion (per the implementation-graph T18 entry, the Status section has read-only alerts and per-row Refresh / Disconnect forms but no top-level Refresh-now button).
...
- **D. Visual capture (committed)**: T16 (`tests/e2e-browser/screenshots/admin/users/{users-after-invite.png, users-expanded.png, users-self-invite-error.png, users-self-row-disabled.png}`) and T17 (`tests/e2e-browser/screenshots/admin/topics/{topics-after-approve.png, topics-after-reject.png, topics-after-retire.png, topics-expanded.png, topics-retire-confirm.png, topics-self-action-disabled.png}`) have their committed-baseline PNGs on disk. **T18 (Admin Status) — `n/a — pending follow-up`; the PNG capture set is recorded in `tests/e2e-browser/screenshots/admin/status/CLOSURE_SUMMARY.md` but no per-state PNGs are committed under `tests/e2e-browser/screenshots/admin/status/` today; the row does not list `status-*.png` paths that are not on disk. **T19 (Admin end-to-end) — `n/a — pending follow-up`; the Admin journey Playwright spec `tests/e2e-browser/journeys/admin/end-to-end.spec.ts` is committed and the corresponding `tests/e2e-browser/screenshots/admin/end-to-end-CLOSURE_SUMMARY.md` is on disk but the per-state PNGs and the WebM screencast under the `capture` `workflow_dispatch` lane are a documented follow-up.
- **E. Visual capture (workflow artifact)**: `workflow_dispatch` `visual-regression.yml` lane per AGENTS.md line 65; the per-PR WebM artifact URL lives in the Admin journey's `workflow_dispatch` run summary when the lane next runs.
...
- **G. Corrective comment + Closure PR**: primary `## Closure Evidence` record at [issue #19 comment 5099921221](https://github.com/rafaelromao/slotmerge/issues/19#issuecomment-5099921221); audit-repair strengthening comment posted at [issue #19 comment 5111930358](https://github.com/rafaelromao/slotmerge/issues/19#issuecomment-5111930358) (with framing clarification at [issue #19 comment 5112191425](https://github.com/rafaelromao/slotmerge/issues/19#issuecomment-5112191425)); **no single closing PR owns this issue** (the sub-PRD closes via the T25 batch).

### C6. [#279](https://github.com/rafaelromao/slotmerge/issues/279) Define rendered-screen and browser-journey completion gates

- **Existing `## Resolution` record**: the existing [issue #279 resolution comment](https://github.com/rafaelromao/slotmerge/issues/279#issuecomment-5025329867) carries the human-confirmed gates recorded from the grilling session; it remains the primary resolution record.
...
- **G. Corrective comment + Closure PR**: existing `## Resolution` record at [issue #279 comment 5025329867](https://github.com/rafaelromao/slotmerge/issues/279#issuecomment-5025329867); audit-repair strengthening comment posted at [issue #279 comment 5111930904](https://github.com/rafaelromao/slotmerge/issues/279#issuecomment-5111930904) (with framing clarification at [issue #279 comment 5112191545](https://github.com/rafaelromao/slotmerge/issues/279#issuecomment-5112191545)); **no single closing PR owns this issue** (the deliverable is the AGENTS.md update merged by the closure-evidence-repair chain).
...
The Admin Topics capture set (T17) is committed under `tests/e2e-browser/screenshots/admin/topics/` (six per-state PNGs). The Admin Status PNG capture set (T18) remains a documented follow-up at `tests/e2e-browser/screenshots/admin/status/CLOSURE_SUMMARY.md`. The Admin journey end-to-end (T19) Playwright spec is committed at `tests/e2e-browser/journeys/admin/end-to-end.spec.ts` and the closure summary at `tests/e2e-browser/screenshots/admin/end-to-end-CLOSURE_SUMMARY.md`; the capture run's PNGs and WebM are a pending follow-up. By the audit window two of the three Admin surfaces (T16 / T17) had landed visual-capture evidence; T18 and T19 visual-capture evidence is partial.
