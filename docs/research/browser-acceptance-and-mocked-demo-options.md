...
5. **Auth setup is via direct DB session seeding for ordinary journeys.** The `sessions` schema is `src/db/schema.ts:55-66`; existing tests already insert rows into `sessions` and call `sealSessionCookie({ sessionId })` to obtain a sealed cookie (`tests/auth-session-route.test.ts:61`; `tests/calendar-action-required-email-wiring-fixtures.ts:80`; note: `tests/availability-overrides-route.test.ts` was deleted in T22). Playwright `globalSetup` should adopt the same pattern: insert one row per role into `sessions`, seal a cookie, and write `storageState` files via `await page.context().storageState({ path })` (`https://playwright.dev/docs/auth`, accessed 2026-07-20). This is not a generic auth bypass endpoint; it is the same direct-DB seeding that the unit/E2E suite already does.
...
- Existing direct-DB session seeding pattern: `tests/auth-session-route.test.ts:61`, `tests/calendar-action-required-email-wiring-fixtures.ts:80` (note: `tests/availability-overrides-route.test.ts` was deleted in T22).
