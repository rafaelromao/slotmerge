...
describe("syncCalendarConnection", () => {
  it("early-returns (no-op) when contributingCalendarIds is empty", async () => {
    ...
    await syncCalendarConnection({
      ...
      userId: "user-1",
      fetchImpl: fetch,
      busyIntervalRepository: { upsertBatch, deleteByConnectionId: vi.fn(), findByUserIdAndDateRange: vi.fn(), deleteExpiredBefore: vi.fn() },
      recordFailure: vi.fn(),
      clock: () => fixedNow,
    });
  ...
  it("calls upsertBatch with mock busy intervals in mock provider mode", async () => {
    ...
    const [intervals] = upsertBatch.mock.calls[0] as [ImportedBusyIntervalRecord[]];
    ...
    expect(["busy", "out-of-office", "tentative"]).toContain(intervals[0]?.status);
  ...
  it("produces deterministic mock intervals for the same connectionId", async () => {
    ...
    await syncCalendarConnection({
      ...
      userId: "user-1",
      fetchImpl: fetch,
      busyIntervalRepository: { upsertBatch: upsertBatch1, deleteByConnectionId: vi.fn(), findByUserIdAndDateRange: vi.fn(), deleteExpiredBefore: vi.fn() },
      recordFailure: vi.fn(),
      clock: () => fixedNow,
    });
    ...
    await syncCalendarConnection({
      ...
      userId: "user-1",
      fetchImpl: fetch,
      busyIntervalRepository: { upsertBatch: upsertBatch2, deleteByConnectionId: vi.fn(), findByUserIdAndDateRange: vi.fn(), deleteExpiredBefore: vi.fn() },
      recordFailure: vi.fn(),
      clock: () => fixedNow,
    });

    const intervals1 = upsertBatch1.mock.calls[0][0] as ImportedBusyIntervalRecord[];
    const intervals2 = upsertBatch2.mock.calls[0][0] as ImportedBusyIntervalRecord[];
    expect(intervals1[0]?.startAt.getTime()).toBe(intervals2[0]?.startAt.getTime());
