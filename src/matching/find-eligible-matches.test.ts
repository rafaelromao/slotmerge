...
describe("hasFullDurationCoverage", () => {
  it("returns true when availability covers the full slot duration", () => {
    const intervals: Interval[] = [
      { startUtc: new Date("2026-07-13T09:00:00Z"), endUtc: new Date("2026-07-13T17:00:00Z") },
    ];
    const slotStart = new Date("2026-07-13T10:00:00Z");
    const durationMinutes = 60;
    expect(hasFullDurationCoverage(intervals, slotStart, durationMinutes)).toBe(true);
  });

  it("returns false when availability ends before slot ends", () => {
    const intervals: Interval[] = [
      { startUtc: new Date("2026-07-13T09:00:00Z"), endUtc: new Date("2026-07-13T10:30:00Z") },
    ];
    const slotStart = new Date("2026-07-13T10:00:00Z");
    const durationMinutes = 60;
    expect(hasFullDurationCoverage(intervals, slotStart, durationMinutes)).toBe(false);
  });

  it("returns true when multiple merged intervals cover the slot", () => {
    const intervals: Interval[] = [
      { startUtc: new Date("2026-07-13T09:00:00Z"), endUtc: new Date("2026-07-13T12:00:00Z") },
      { startUtc: new Date("2026-07-13T13:00:00Z"), endUtc: new Date("2026-07-13T17:00:00Z") },
    ];
    const slotStart = new Date("2026-07-13T10:00:00Z");
    const durationMinutes = 60;
    expect(hasFullDurationCoverage(intervals, slotStart, durationMinutes)).toBe(true);
  ...
  it("returns true for exact coverage", () => {
    const intervals: Interval[] = [
      { startUtc: new Date("2026-07-13T10:00:00Z"), endUtc: new Date("2026-07-13T11:00:00Z") },
    ];
    const slotStart = new Date("2026-07-13T10:00:00Z");
    const durationMinutes = 60;
    expect(hasFullDurationCoverage(intervals, slotStart, durationMinutes)).toBe(true);
  ...
  it("returns false when slot duration extends beyond last interval end", () => {
    const intervals: Interval[] = [
      { startUtc: new Date("2026-07-13T09:00:00Z"), endUtc: new Date("2026-07-13T11:00:00Z") },
    ];
...
describe("findEligibleMatches", () => {
  const createMockDeps = (overrides: Partial<MatchingDependencies> = {}): MatchingDependencies => {
    return {
      listSelectedTopicIds: () => Promise.resolve([]),
      computeEffectiveAvailability: () => [],
      getUserAvailabilityData: () => Promise.resolve({
        profileTimezone: "UTC",
        bufferMinutes: 0,
        windows: [],
        overrides: [],
  ...
  it("excludes users missing a selected topic (AC1)", async () => {
    const deps = createMockDeps({
      listSelectedTopicIds: (userId: string) => Promise.resolve(
        userId === "user-1" ? ["topic-a", "topic-b"] :
        userId === "user-2" ? ["topic-a"] : []
      ),
      isUserEligibleForSearch: () => Promise.resolve(true),
    });
    const result = await findEligibleMatches(
  ...
  it("excludes users who are not eligible (AC3/4/5)", async () => {
    const deps = createMockDeps({
      listSelectedTopicIds: () => Promise.resolve(["topic-a"]),
      isUserEligibleForSearch: (userId: string) => Promise.resolve(userId === "user-1"),
    });
    const result = await findEligibleMatches(
      {
        organizerId: "organizer",
