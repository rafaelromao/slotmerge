...
async function seedJourneyFixture(options: { withStaleConnection?: boolean } = {}): Promise<void> {
  const db = getDb();
  await db.execute(
    sql`DELETE FROM searches WHERE organizer_id = ${ORGANIZER_ID}`,
  );
  await seedAll(db);
...
function locateStaleSlot(page: import("@playwright/test").Page): import("@playwright/test").Locator {
  return page.locator('[data-testid^="slot-"][data-stale="true"]').first();
}
...
test.describe("slot details drawer surface", () => {
  ...
  test("happy path: opens a stale Slot with accessible dialog semantics, focus trap, Escape close, and the no-booking footer", async ({
    ...
    const staleSlot = locateStaleSlot(page);
...
test.describe("search history surface", () => {
  ...
  test("happy path: lists the source snapshot, opens it, and confirms the source id remains readable", async ({
    ...
    const sourceRow = historyRows.filter({
      has: page.locator(
        `[data-testid="search-history-open-snapshot"][href*="${sourceSearchId}"]`,
      ),
    });
    ...
    await sourceRow.getByTestId("search-history-open-snapshot").click();
...
test.describe("rerun surface", () => {
  ...
  test("happy path: form → result → drawer → history → rerun → reopened source", async ({
    page,
  }) => {
    await page.clock.install({ time: FIXED_DATE });
    const sourceSearchId = await runSearchThroughForm(page);
    ...
    await sourceRow.getByTestId("search-history-rerun").click();
    ...
    const sourceRowAfter = twoSnapshots.filter({
      has: page.locator(
        `[data-testid="search-history-open-snapshot"][href*="${sourceSearchId}"]`,
      ),
    });
    const rerunRowAfter = twoSnapshots.filter({
      has: page.locator(
        `[data-testid="search-history-open-snapshot"][href*="${rerunSearchId}"]`,
      ),
    });
    await expect(sourceRowAfter).toHaveCount(1);
    await expect(rerunRowAfter).toHaveCount(1);
