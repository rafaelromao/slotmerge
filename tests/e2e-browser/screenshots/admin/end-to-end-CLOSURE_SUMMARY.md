...
| `tests/e2e-browser/screenshots/admin/{users,topics,status}/<state>.png` baselines committed for every named visible state                                                                                                                               | 21 per-state PNGs committed under the three surface subdirectories (11 inherited from T16/T17/T18 + 10 new T19-named captures from this PR); full table in `tests/e2e-browser/screenshots/admin/README.md`; the `CAPTURE=true` capture project produces them and `visual-regression.yml` commits any drift on `workflow_dispatch` |
...
The 10 new T19 per-state PNGs (the ones the end-to-end spec names
with the `invite-banner`, `role-changed`, `suspend-confirm`,
`suspended`, `reinstated`, `approve-proposal`, `reject-proposal`,
`retire-confirm`, `retired`, and `status/expanded` states) are
committed to `tests/e2e-browser/screenshots/admin/{users,topics,status}/`
in this PR. The T19-named captures that overlap with T16/T17/T18
captures (e.g. `users/invite-banner` ≈ `users/users-after-invite`)
are placeholders that the `visual-regression.yml` `workflow_dispatch`
lane regenerates from the end-to-end spec on its first run after
merge.
