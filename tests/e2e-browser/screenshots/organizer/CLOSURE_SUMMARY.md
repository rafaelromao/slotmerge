...
| Organizer `storageState` (`playwright/.auth/organizer.json`)                | Top-level `test.use({ storageState: "playwright/.auth/organizer.json" })` of the spec |
| Self-contained `test.describe` blocks (failure points at the right surface)  | Each `test.describe` runs in Playwright's default mode with its own `beforeEach` reseed; the outer describe group does not use `mode: "serial"`, so a failure in one surface does not block the others from running |
| `tests/e2e-browser/screenshots/organizer/{search-form,search-result,search-history}/<state>.png` baselines | `tests/e2e-browser/screenshots/organizer/README.md` enumerates the 11 required named states; the `CAPTURE=true` capture project produces them and `visual-regression.yml` commits them on `workflow_dispatch` |
...
| Green on local stack + `workflow_dispatch` lane; PR CI does not run journey | `browser-tests.yml` and `visual-regression.yml` run on `workflow_dispatch` only; PR CI runs `pnpm typecheck && pnpm lint && pnpm format:check && pnpm test && pnpm build` (Vitest only) |
